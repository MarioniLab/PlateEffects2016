...
We further define $\theta_{jgk}$ as the random variable for the bias in cell $j$, and $\delta_{ik}$ as the random variable for the gene-/plate-specific effect in gene $i$ and plate $k$.
$Y_{ijkg}$ is conditionally distributed as
\[
    Y_{ijkg} | \delta_{ik}, \theta_{jgk} \sim \mbox{NB}(\delta_{ik}\theta_{jgk}\mu_{ig}, \varphi_i)
\]
where $\mu_{ig}$ is the expected read count of gene $i$ in group $g$ and $\varphi_i$ is the NB dispersion for gene $i$.
All counts are assumed to be independently sampled, conditional on the realized values of $\theta_{jgk}$ and $\delta_{ik}$.
...
Similarly, $\theta_{jgk}$ is sampled from some (empirically defined, see below) distribution with a mean of unity, i.e., the bias averages out across many cells.
We also assume that $\theta_{jgk}$ and $\delta_{ik}$ are independent of each other.
Having means of unity for both $\theta_{jgk}$ and $\delta_{ik}$ is not strictly necessary but simply ensures that $E(Y_{ijkg})=\mu_{ig}$, consistent with its definition.

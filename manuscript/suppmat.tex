...
This parametrization ensures that $E(\Delta_{ik}) = 1$, i.e., the plate effect averages out across many plates.
Similarly, $\theta_j$ is sampled from some distribution with a mean of unity (i.e., the bias averages out across many cells) which is independent of $\Delta_{ik}$.
Thus, the mean count across all cells and plates in group $g$ will be $\mu_{ig}$, as defined.

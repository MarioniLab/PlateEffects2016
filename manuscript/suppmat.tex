...
\subsection{Overview}
We simulate counts from a NB-Gamma mixture, where the counts are conditionally NB-distributed and the plate effect is Gamma-distributed.
For count $Y_{ij}$ of gene $i$ in cell $j$ in plate $k$ of group $g$, we assume that
\[
    Y_{ij} | \delta_{ik} \sim \mbox{NB}(\delta_{ik}\theta_j\mu_{ig}, \varphi_i)
\]
where $\theta_j$ is the cell-specific bias, $\delta_{ik}$ is the plate effect, $\mu_{ig}$ is the expected count in this group, and $\varphi_i$ is the NB dispersion for this gene.
We denote the random variable for $\delta_{ik}$ as $\Delta_{ik}$, where $\log(\Delta_{ik})$ is normally distributed with variance $\sigma^2$ and mean $-\sigma^2/2$.
This parameterization ensures that $E(\Delta_{ik}) = 1$, such that the mean count of each gene will be $\mu_{ig}$.
Sampling of $Y_{ij}$ is also conditionally independent of $\Delta_{ik}$.
...
For the cell-specific biases, we applied the deconvolution method \citep{lun2016pooling} using each culture type as a cluster.
This yielded size factors that represent estimates of $\theta_j$ (Figure~\ref{subfig:realsize}).
For the mean count of each gene, we fitted an intercept-only GLM to the counts from all cells, using the mglmOneGroup function in edgeR with mean-centered log-size factors as offsets.
...
The tagwise estimate was used as $\varphi_i$ for each gene (Figure~\ref{subfig:realdisp}).
...
%        \includegraphics[width=\textwidth]{../reference/results_ESpresso/platevar.pdf}
    ...
    \caption{Estimates of the simulation parameters from the mESC data set.
        ...
        (\subref{subfig:realplate}) Histogram of the estimated variances of the plate effect across all genes, where the red line indicates the average estimate.
    }
\end{figure}
...
Offsets were defined as log-transformed size factors for all cells.
...
This, along with the mean and dispersion of the chosen gene, was used to define the sampling distribution of $Y_{ij}$ for each cell in plate $k$.
We repeated this process for each plate to obtain a matrix of counts for all genes in all cells.
...
\begin{color}{red}
For GLMMs, the glmer.nb function in the lme4 package v1.1-10 was used to fit a NB GLMM to the counts for each gene.
The group was set as a fixed effect while the plate of origin was set as a random effect for each cell.
...
(Assessing significance via bootstrapping is not practical for thousands of genes.) 
...
Dependencies caused by plate effects are avoided as plate-specific expression is explicitly modelled in the one-way layout.
...
As an aside, what happens when each plate contains several \textit{a priori} undefined biological subpopulations with different gene expression profiles?
...
Each count sum can be considered to be independently sampled from a distribution with this mean, due to the independence of $\delta_{ik}$ and conditional independence of the counts.

...
We define $Y_{ijkg}$ as the random variable for the count of gene $i$ in cell $j$ in plate $k$ of group $g$.
We further define $\theta_j$ as the random variable for the bias in cell $j$, and $\delta_{ik}$ as the random variable for the gene-/plate-specific effect in gene $i$ and plate $k$.
$Y_{ijkg}$ is conditionally distributed as
\[
    Y_{ijkg} | \delta_{ik}, \theta_j \sim \mbox{NB}(\delta_{ik}\theta_j\mu_{ig}, \varphi_i)
\]
where $\mu_{ig}$ is the expected read count of gene $i$ in group $g$ and $\varphi_i$ is the NB dispersion for gene $i$.
All counts are assumed to be independently sampled, conditional on the realized values of $\theta_j$ and $\delta_{ik}$.
...
Similarly, $\theta_j$ is sampled from some (empirically defined, see below) distribution with a mean of unity, i.e., the bias averages out across many cells.
We also assume that $\theta_j$ and $\delta_{ik}$ are independent of each other.
Having means of unity for both $\theta_j$ and $\delta_{ik}$ is not strictly necessary but simply ensures that $E(Y_{ijkg})=\mu_{ig}$, consistent with its definition.
...
Specifically, the mean of the NB component was set to $\delta_{ik}\theta_j\mu_{i0}'$, the dispersion was set to $\varphi_i'$ and the zero probability was set to $p'_i$.
...
The bias $\theta_j$ for each cell was sampled with replacement from the set of size factor estimates obtained from the mESC data set.
...
\begin{align*}
    ...
    &= E(S_{ik})^2 \mbox{var}(\delta_{ik}) + E(S_{ik}) + \left\{ \varphi_i \frac{\sum_l N_l \theta_l^2\mu_{ig}^2}{ (\sum_l N_l \theta_l\mu_{ig})^2}  \right\} E(S_{ik})^2 E(\delta_{ik}^2) \;. 
\end{align*}
...
\[
    \lim_{\substack{N_{l_1} \to \infty \\ \dots \\ N_{l_b} \to \infty}} \frac{\sum_l N_l \theta_l^2\mu_{ig}^2}{ (\sum_l N_l \theta_l\mu_{ig})^2} 
    = \sum_l \lim_{\substack{N_{l_1} \to \infty \\ \dots \\ N_{l_b} \to \infty}} \frac{N_l \theta_l^2\mu_{ig}^2}{ (\sum_l N_l \theta_l\mu_{ig})^2} 
= 0
\]

...
For edgeR, analyses were performed with and without EB shrinkage.
...
For voom, counts were converted to log-counts per million (log-CPMs) and precision weights were computed from a fitted mean-variance trend.
A linear model was fitted to the log-CPMs and weights for each gene using the lmFit function in limma.
...
In the simulation, all libraries are of the same size and all genes are of the same length, 
    so the log-counts are functionally equivalent to the log-transcripts per million used in the original description of the algorithm \citep{finak2015mast}.
A hurdle model was fitted to the log-counts using the biological group as the factor of interest and the number of non-zero counts in each library as an additional covariate.
...
\]
for positive values of $\theta_j$ and $\mu_{ig}$, such that the ratio of the latter to the former will always decrease for an increasing number of cells.
...
Even if a more stable average measure is used (e.g., the arithmetic mean), the median ratio for many libraries will be equal to zero if more than 50\% of the counts of that library are zero.
This is not a sensible scaling factor for obtaining normalized expression values.

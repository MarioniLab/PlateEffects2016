...
\caption{
    ...
    The difference in the enrichment of factors in the top set of 250 genes between summed and single-cell analyses was tested using Fisher's exact test, yielding a $p$-value of 0.030 (using the total numbers across both comparisons). 
}
\end{figure}
...
\begin{figure}[p]
    ...
    \end{center}
\caption{
    Expression profiles of DE genes between 2i and serum (top) or between a2i and serum conditions (bottom) in the mESC data set, for the top genes detected by QL edgeR on summed counts only.

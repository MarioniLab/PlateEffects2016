...
2. For contrasts _between_ subpopulations, the genes involved in the identification must not be the ones that are tested.
This avoids problems with double-dipping into the data, assuming that the clusters are precisely identified.
If clustering is not precise, you may get double-dipping due to correlations in the errors.
For example, you can get skewed axes from PCA that explain much more technical noise than expected by chance.
Testing for DE against these PCs would probably result in rejections due to correlation with the errors.

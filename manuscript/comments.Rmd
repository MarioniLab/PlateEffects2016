...
This also avoids systematic differences in clustering, caused by the differential presence of other subpopulations between conditions.
One would do this anyway, as it's too hard to match up clusters afterwards.
2. The genes involved in the identification must not be the ones that are tested.
This avoids problems with double-dipping into the data, assuming that the clusters are precisely identified.
(If not, you may get double-dipping due to correlations in the errors.)

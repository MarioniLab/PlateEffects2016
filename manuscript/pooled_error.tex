...
Loss of type I error control is caused by hidden dependencies between cells on the same plate.
Most DE analysis methods assume that replicate samples (i.e., cells) are independent, given the conditions specified in the one-way layout.
This is not the case when a plate effect is present in the data but is ignored during statistical modelling.
Counts will associate with the plate of origin for each cell rather than with the condition of that cell.
These dependencies between counts for all cells on the same plate are present because the mean expression in those cells is modified by the same plate effect.
This reduces the amount of information in the data set as cells on the same plate are effectively redundant.
Assuming independence will overstate the amount of information for variance estimation, resulting in overconfident inferences and liberalness.
...
Another contributing factor is the low number of plates, which may result in biased estimates for the consensus correlation.
...
Values of $\lambda_{ik}$ and $\varphi_{ik}$ were defined based on $\mu_{il_1}$ and $\mu_{il_2}$ for each gene $i$ in each plate.

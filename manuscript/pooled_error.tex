...
% Gaublomme's paper has multiple C1 chips for CNS/LN across batches (the DE test is done with empirical subpopulations, but still ignores the plate effect as it uses a one-way layout).
% Kowalczyk's paper has 192 cells for each condition, from 96-well plates of SMART-seq.
...
A common strategy ignores the plate effects and treats cells from different plates in the same group as replicates.
This is most obvious in a one-way layout but also applies to multi-factor designs involving some level of replication across plates.
The estimated variance will include the conditional variance between cells on given plate, i.e., var$(X_{ik})$, as well as the variance of $\delta_{ik}$ between plates.
DE genes can then be detected between groups using software designed for bulk RNA-seq data (e.g., DESeq) or with dedicated single-cell methods (e.g., Monocle).
This approach has been used in a number of scRNA-seq studies to detect DE across plates or chips \citep{kolod2015single,gaublomme2015single,kowalczyk2015single}.

...
\revised{Many studies ignore plate effects during the DE analysis and treat} all cells from the same group as replicates, e.g., \cite{kolod2015single}.
This strategy \revised{is} not statistically rigorous as the variability across cells on the same plate will not be modelled properly.
...
In the following text, we refer to $\mbox{var}(Y_{ij}|\delta_{ik},\theta_j)$ as the conditional variance of the count.
Similarly, the conditional expectation is defined as
\[
    E(Y_{ij}|\delta_{ik},\theta_j) = \delta_{ik}\theta_j\mu_{ig} \;.
\]
This expression represents the impact of plate effects on the data.
For each gene, the mean for all cells on each plate is scaled by a different value of $\delta_{ik}$, leading to systematic differences between plates.
...
This is because normalization computes a single scaling factor for each cell to correct for $\theta_j$ -- in contrast, the plate effect varies across genes and cannot be removed by a single factor.
\end{color}
...
However, this is complicated by the presence of \revised{plate effects that are confounded by the biological groups.
Consider an experimental design involving one plate in each of two groups.
...
Parameters of both distributions were estimated from the real data.
...
The GLMM approach is more general, and uses NB distributions to model counts with random plate and fixed group effects.
\end{color}
...
\revised{The GLMM approach and voom with correlations exhibit the best performance of all tested methods in the presence of a plate effect (Figure~\ref{fig:platefail}).
...
For voom, this may also be compounded by the inadequacy of the Normal model for highly discrete counts, particularly in scRNA-seq data where low-abundance genes have many zero counts.}

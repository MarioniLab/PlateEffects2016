...
For all methods, normalization was performed using the deconvolution method for the single-cell counts \citep{lun2016pooling} or the DESeq method for the summed counts for each plate \citep{anders2010differential}.
...
\end{center}
\caption{
    Proportion of genes with DE $p$-values below the nominal threshold of 0.01 (red line) for the 2i vs. serum and a2i vs. serum comparisons, after applying each analysis method with and without summation to the label-swapped data.
    Proportions represent type I error rates as all null hypotheses are true.
}
\label{fig:realswap}
\end{figure}

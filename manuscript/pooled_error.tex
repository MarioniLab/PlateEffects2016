...
The DE analysis itself is typically performed using software packages such as edgeR \citep{robinson2010edgeR} and DESeq \citep{anders2010differential}, which were designed for bulk data;
...
edgeR was run twice, using either the likelihood ratio test (LRT) \citep{mccarthy2012differential} or the quasi-likelihood (QL) framework \citep{lund2012detecting}.
...
This subset contains all cells used in Figure X in \citep{scialdone2015single}, and was originally selected by clustering on the single-cell expression profiles 

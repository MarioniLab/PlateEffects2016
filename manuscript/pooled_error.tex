...
The plate effect $\delta_{ik}$ is sampled from some independent distribution $Z$ where $E(Z) = 1$, which ensures that $E(y_{ij}) = \mu_{il}$.
...
The plate effects themselves are sampled from a log-normal distribution, i.e., $Z = \ln \mathcal{N}(-\sigma^2/2, \sigma^2)$.
...
Note that this evaluation is only possible for methods that compute $p$-values for each gene -- 
    Bayesian methods \citep{vallejos2015basics,kharchenko2014bayesian} are not comparable as they generate posterior probabilities, and so are not tested here.
...
voom is inherently more conservative \citep{law2014voom}, which results in lower observed rates and restoration of type I error control.
...
\begin{figure}[!p]
\begin{center}
\includegraphics[width=0.48\textwidth,trim=0mm 0mm 0mm 5mm,clip]{failsim_with_raw.eps}
\includegraphics[width=0.48\textwidth,trim=0mm 0mm 0mm 5mm,clip]{failsim_without_raw.eps}
\end{center}
\caption{
    ...
    Each error bar represents the standard error of the average.
    The threshold of 0.01 is represented by the horizonal dashed line.
...
\begin{figure}[!p]
\begin{center}
\includegraphics[width=0.48\textwidth,trim=0mm 0mm 0mm 15mm,clip]{failsim_with_sum.eps}
\end{center}
\caption{
    Observed type I error rate for each method after summation in simulations with a plate effect.
    Error rates are shown on a log scale and represent the average across 10 simulation iterations.
    Error bars represent standard errors, and the threshold of 0.01 is represented by the horizontal dashed line.
}
\label{fig:platesum}
\end{figure}
...
\begin{figure}[!p]
\begin{center}
\includegraphics[width=0.48\textwidth,trim=0mm 0mm 0mm 10mm,clip]{quantile_with.eps}

...
\newcommand{\suppimplementation}{1i}
...
edgeR was run twice, using either the likelihood ratio test (LRT) \cite{mccarthy2012differential} or the quasi-likelihood (QL) framework \cite{lund2012detecting}.
...
\begin{figure}[tbp]
\begin{center}
\includegraphics[width=0.48\textwidth,trim=0mm 0mm 0mm 15mm,clip]{failsim_with_sum.pdf}
\end{center}
\caption{
...
This means that the count sums across all plates will have near-identical mean-variance relationships, as described above for the pseudo-counts.
Indeed, the similarity to the results for pseudo-count summation suggests that any plate-specific differences in the relationships will not affect model performance after direct summation.

This can be explained more precisely by examining the the mean-variance relationship of Equation~\ref{eqn:meanvar} for the simulated data.
The plate-specific term in the relationship is inversely proportional to the number of cells $N_k$.
For a large value of $\mbox{var}(\delta_{ik})$, the plate-specific contribution to the variance will approach zero as $N_k$ increases. 
This is also true if the cells on the plate have different library sizes.
Consider the previous notation where $\lambda_{ij}$ represents the conditional mean for cell $j$.
The conditional variance becomes
\[
    \mbox{var}(s_{ik} | \delta_{ik}) = \sum_{N_k} \lambda_{ij}  + \varphi_{i} \sum_{N_k} \lambda_{ij}^2 \;,
\]
such that the total variance of the count sum across all plates can be written as 
\begin{align*}
    \mbox{var}(s_{ik}) &= E(s_{ik})^2 \mbox{var}(\delta_{ik}) + E(s_{ik}) + \left( \varphi_i \frac{\sum_{N_k} \theta_j^2\mu_{ig}^2}{E(s_{ik})^2}  \right) E(s_{ik})^2 E(\delta_{ik}^2) \\
...
\end{align*}
Again, the relative contribution of the plate-specific term will approach zero as $N_k$ increases (for positive values of $\theta_j$ and $\mu_{ig}$).
Summation will be similarly robust to plate- or even cell-specific values for the dispersion of the NB distribution.
This suggests that quantile adjustment prior to summation is unnecessary.

...
\subsection{The effect of variability between cells on the count sum}
...
This increases the total variance and decreases the relative significance of DE.
...
If more cells contribute to the sum, cell-to-cell variability will cease to affect the total variance.
...
This philosophy is almost the exact opposite of that in microarray analyses involving technical replicates 
    -- there, the choice is between averaging the replicate signals or including the replicate-to-replicate variability in the analysis,
    and the latter is often preferred to mitigate the effect of artifacts in one or more replicates \citep{smyth2005use}.
The difference in strategies is due to the fact that variability between technical replicates should be minimal whereas variability between cells depends on the biology.

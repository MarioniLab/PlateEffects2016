...
Further define $\theta_{jgk}$ as a random variable representing the bias in cell $j$.
...
$Y_{ijkg}$ is sampled independently for each gene in each cell, conditional on the observed values of $\delta_{ik}$ and $\theta_{jgk}$.
In the following text, we refer to $\mbox{var}(Y_{ijkg}|\delta_{ik},\theta_{jgk})$ as the conditional variance, while the conditional expectation is defined as
\[
    E(Y_{ij}|\delta_{ik},\theta_{jgk}) = \delta_{ik}\theta_{jgk}\mu_{ig} 
\]
...
This is because these normalization methods compute a single scaling factor for each cell to adjust for the cell-specific $\theta_{jgk}$.
...
For all methods, the experimental design was parametrized as a one-way layout with two groups.
